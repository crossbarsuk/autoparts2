...
<div id="primary_block" class="clearfix">
	...
	<div id="pb-left-column">
        ...
        <a title="{l s='Заказать звонок'}" class="gray-btn" href="#">{l s='Заказать звонок'}</a>
			</p>
      ...
      <table width="100%">
        ...
        {foreach from=$features item=feature}
          {if isset($feature.value)}
            ...
            <tr class="{if $odd}even{else}odd{/if}"><td>{$feature.name|escape:'htmlall':'UTF-8'}</td><td>{$feature.value|escape:'htmlall':'UTF-8'}</td></tr>
          {/if}
        {/foreach}
      </table>
    </div>
...
{if (isset($product) && $product->description) || (isset($accessories) && $accessories) || (isset($HOOK_PRODUCT_TAB) && $HOOK_PRODUCT_TAB) || (isset($attachments) && $attachments) || isset($product) && $product->customizable}
