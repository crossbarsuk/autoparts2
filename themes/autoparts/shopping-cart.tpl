<div class="block">
{capture name=path}{l s='Ваш заказ:'}{/capture}
{*{include file="$tpl_dir./breadcrumb.tpl"}*}

<h1 id="cart_title">{l s='Ваш заказ:'}</h1>
...
{if isset($empty)}
  <div class="empty_cart">
    ...
    <div>
      <b>{l s='Your shopping cart is empty.'}</b>
      <img alt="min cart" src="/themes/autoparts/img/min_cart_ico.png">
      <p>{l s='Для добавления деталей в корзину нажмите на ссылку с изображением корзины в таблице результатов поиска .'} </p>
    </div>
  </div>
...
{/if}
<p style="display: none">{l s='Your shopping cart contains:'} <span id="summary_products_quantity">{$productNumber} {if $productNumber == 1}{l s='product'}{else}{l s='products'}{/if}</span></p>
<div id="order-detail-content" class="table_block">
	<table id="cart_summary" class="std">
        ...
        <td class="total_price_container">{l s='Total'} </td>
				{if $use_taxes}
...
<p class="cart_navigation">
	{if !$opc}
		...
		{/if}
	{/if}
	<a href="{if (isset($smarty.server.HTTP_REFERER) && strstr($smarty.server.HTTP_REFERER, 'order.php')) || isset($smarty.server.HTTP_REFERER) && strstr($smarty.server.HTTP_REFERER, 'order-opc') || !isset($smarty.server.HTTP_REFERER)}{$link->getPageLink('index')}{else}{$smarty.server.HTTP_REFERER|escape:'htmlall':'UTF-8'|secureReferrer}{/if}" class="gray-btn" title="{l s='Continue shopping'}">&laquo; {l s='Continue shopping'}</a>
</p>
