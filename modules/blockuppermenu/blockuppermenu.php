...
require (dirname(__FILE__).'/menuupperlinks.class.php');

class Blockuppermenu extends Module
{
	private $_menu = '';
	private $_html = '';
	private $user_groups;
	...
	public function __construct()
	{
		...
		$this->displayName = $this->l('Upper menu');
		$this->description = $this->l('Add a new Upper menu of your e-commerce website.');
	}

	public function install()
	{
		if (!parent::install() ||
			!$this->registerHook('upperMenu') ||
			!$this->registerHook('header') ||
			!Configuration::updateGlobalValue('MOD_BLOCKUPPERMENU_ITEMS', 'CAT1,CMS1,CMS2,PRD1') ||
	...
	public function installDb()
	{
		...
		CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'linksmenuupper` (
			`id_linksmenuupper` INT UNSIGNED NOT NULL AUTO_INCREMENT PRIMARY KEY,
			`id_shop` INT(11) UNSIGNED NOT NULL,
			`new_window` TINYINT( 1 ) NOT NULL,
			INDEX (`id_shop`)
		) ENGINE = '._MYSQL_ENGINE_.' CHARACTER SET utf8 COLLATE utf8_general_ci;') &&
			Db::getInstance()->execute('
			 CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'linksmenuupper_lang` (
			`id_linksmenuupper` INT(11) UNSIGNED NOT NULL,
			`id_lang` INT(11) UNSIGNED NOT NULL,
			`id_shop` INT(11) UNSIGNED NOT NULL,
			`label` VARCHAR( 128 ) NOT NULL ,
			`link` VARCHAR( 128 ) NOT NULL ,
			INDEX ( `id_linksmenuupper` , `id_lang`, `id_shop`)
		) ENGINE = '._MYSQL_ENGINE_.' CHARACTER SET utf8 COLLATE utf8_general_ci;'));
	}
	...
	{
		if (!parent::uninstall() ||
			!Configuration::deleteByName('MOD_BLOCKUPPERMENU_ITEMS') ||
			!$this->uninstallDB())
			return false;
		return true;
	}
	...
	{
		Db::getInstance()->execute('DROP TABLE `'._DB_PREFIX_.'linksmenuupper`');
		Db::getInstance()->execute('DROP TABLE `'._DB_PREFIX_.'linksmenuupper_lang`');
		return true;
	}
	...
	public function getContent()
	{
		...
		if (Tools::isSubmit('submitBlockuppermenu'))
		{
			if (Configuration::updateValue('MOD_BLOCKUPPERMENU_ITEMS', Tools::getValue('items')))
		...
		else if (Tools::isSubmit('submitBlockuppermenuLinks'))
		{
			...
			{
				MenuUpperLinks::add(Tools::getValue('link'), Tools::getValue('label'), Tools::getValue('new_window', 0), (int)Shop::getContextShopID());
				$this->_html .= $this->displayConfirmation($this->l('The link has been added.'));
			}
			$update_cache = true;
		}
		else if (Tools::isSubmit('submitBlockuppermenuRemove'))
		{
			$id_linksmenuupper = Tools::getValue('id_linksmenuupper', 0);
			MenuUpperLinks::remove($id_linksmenuupper, (int)Shop::getContextShopID());
			Configuration::updateValue('MOD_BLOCKUPPERMENU_ITEMS', str_replace(array('LNK'.$id_linksmenuupper.',', 'LNK'.$id_linksmenuupper), '', Configuration::get('MOD_BLOCKUPPERMENU_ITEMS')));
			$this->_html .= $this->displayConfirmation($this->l('The link has been removed'));
			$update_cache = true;
		}
		else if (Tools::isSubmit('submitBlockuppermenuEdit'))
		{
			$id_linksmenuupper = (int)Tools::getValue('id_linksmenuupper', 0);
			$id_shop = (int)Shop::getContextShopID();

			if (!Tools::isSubmit('link'))
			{
				$tmp = MenuUpperLinks::getLinkLang($id_linksmenuupper, $id_shop);
			...
			{
				MenuUpperLinks::update(Tools::getValue('link'), Tools::getValue('label'), Tools::getValue('new_window', 0), (int)$id_shop, (int)$id_linksmenuupper, (int)$id_linksmenuupper);
		...
		<fieldset>
			...
			<form action="'.Tools::safeOutput($_SERVER['REQUEST_URI']).'" method="post" id="form">
				...
				<div class="margin-form">
					<input type="text" name="items" id="itemsInput" value="'.Tools::safeOutput(Configuration::get('MOD_BLOCKUPPERMENU_ITEMS')).'" size="70" />
				</div>
				</div>
		...
		// BEGIN Menu Upper Links
		$this->_html .= '<optgroup label="'.$this->l('Menu Upper Links').'">';
		$links = MenuUpperLinks::gets($id_lang, null, (int)Shop::getContextShopID());
		foreach ($links as $link)
		{
			if ($link['label'] == '')
			{
				$link = MenuUpperLinks::get($link['id_linksmenuupper'], $default_language, (int)Shop::getContextShopID());
				$this->_html .= '<option value="LNK'.(int)$link[0]['id_linksmenuupper'].'">'.$spacer.$link[0]['label'].'</option>';
			}
			else
				$this->_html .= '<option value="LNK'.(int)$link['id_linksmenuupper'].'">'.$spacer.$link['label'].'</option>';
		}
		$this->_html .= '</optgroup>';
		...
		$this->_html .= '</select><br/>
				...
				<p class="center">
					<input type="submit" name="submitBlockuppermenu" value="'.$this->l('Save	').'" class="button" />
				</p>
			</form>
		</fieldset><br />';
		...
		$this->_html .= '
		<fieldset>
			<legend><img src="../img/admin/add.gif" alt="" title="" />'.$this->l('Add Menu Upper Link').'</legend>
			<form action="'.Tools::safeOutput($_SERVER['REQUEST_URI']).'" method="post" id="form">
...
<div class="margin-form">';
		...
		if (Tools::isSubmit('id_linksmenuupper'))
			$this->_html .= '<input type="hidden" name="id_linksmenuupper" value="'.(int)Tools::getValue('id_linksmenuupper').'" />';

		if (Tools::isSubmit('submitBlockuppermenuEdit'))
			$this->_html .= '<input type="submit" name="submitBlockuppermenuEdit" value="'.$this->l('Edit').'" class="button" />';

		$this->_html .= '
					<input type="submit" name="submitBlockuppermenuLinks" value="'.$this->l('Add	').'" class="button" />
</div>

			</form>
		</fieldset><br />';

		$links = MenuUpperLinks::gets((int)$id_lang, null, (int)Shop::getContextShopID());
		...
		$this->_html .= '
		<fieldset>
			<legend><img src="../img/admin/details.gif" alt="" title="" />'.$this->l('List Menu Upper Link').'</legend>
		...
		foreach ($links as $link)
		{
			$this->_html .= '
					<tr>
						<td>'.(int)$link['id_linksmenuupper'].'</td>
						...
						<td>
							<form action="'.Tools::safeOutput($_SERVER['REQUEST_URI']).'" method="post">
								<input type="hidden" name="id_linksmenuupper" value="'.(int)$link['id_linksmenuupper'].'" />
								<input type="submit" name="submitBlockuppermenuEdit" value="'.$this->l('Edit').'" class="button" />
								<input type="submit" name="submitBlockuppermenuRemove" value="'.$this->l('Remove').'" class="button" />
							</form>
						</td>
					</tr>';
		}
	...
	{
		return explode(',', Configuration::get('MOD_BLOCKUPPERMENU_ITEMS'));
	}
	...
	private function makeMenuOption()
	{
		...
		foreach ($menu_item as $item)
		{
			...
			switch (substr($item, 0, strlen($values[1])))
			{
				...
				case 'LNK':
					$link = MenuUpperLinks::get((int)$id, (int)$id_lang, (int)$id_shop);
					if (count($link))
					{
						if (!isset($link[0]['label']) || ($link[0]['label'] == ''))
						{
							$default_language = Configuration::get('PS_LANG_DEFAULT');
							$link = MenuUpperLinks::get($link[0]['id_linksmenuupper'], (int)$default_language, (int)Shop::getContextShopID());
						}
						$this->_html .= '<option value="LNK'.$link[0]['id_linksmenuupper'].'">'.$link[0]['label'].'</option>';
					}
					break;
				case 'SHOP':
	...
	private function makeMenu()
	{
		...
		foreach ($menu_items as $item)
		{
			...
			switch (substr($item, 0, strlen($value[1])))
			{
				...
				case 'PRD':
					$selected = ($this->page_name == 'product' && (Tools::getValue('id_product') == $id)) ? ' class="selected"' : '';
					$product = new Product((int)$id, true, (int)$id_lang);
				...
				case 'CMS':
					$selected = ($this->page_name == 'cms' && (Tools::getValue('id_cms') == $id)) ? ' class="selected"' : '';
...
//						$this->getCMSMenuItems($category->id);
				...
				case 'MAN':
					$selected = ($this->page_name == 'manufacturer' && (Tools::getValue('id_manufacturer') == $id)) ? ' class="selected"' : '';
					$manufacturer = new Manufacturer((int)$id, (int)$id_lang);
				...
				case 'SUP':
					$selected = ($this->page_name == 'supplier' && (Tools::getValue('id_supplier') == $id)) ? ' class="selected"' : '';
					$supplier = new Supplier((int)$id, (int)$id_lang);
				...
				case 'SHOP':
					$selected = ($this->page_name == 'index' && ($this->context->shop->id == $id)) ? ' class="selected"' : '';
					$shop = new Shop((int)$id);
					if (Validate::isLoadedObject($shop))
					{
						$link = new Link;
				...
				case 'LNK':
					...
					if (count($link))
					{
						...
						{
							$default_language = Configuration::get('PS_LANG_DEFAULT');
							$link = MenuUpperLinks::get($link[0]['id_linksmenuupper'], $default_language, (int)Shop::getContextShopID());
						}
            $selected = $_SERVER['REQUEST_URI'] == $link[0]['link'] ? ' class="selected"' : '';
  ...
  private function isSelected($id_category, $aChildren, $id_lang, $active = true, $id_shop = false) {
    static $iCurCatId = null;
    
    if (!$iCurCatId) {
      $iCurCatId = (int)Tools::getValue('id_category');
    }
    
    if ($iCurCatId == $id_category) {
      return true;
    }
    
    foreach ($aChildren as $aChild) {
      if ($aChild['id_category'] == $iCurCatId || $this->isSelected($aChild['id_category'], Category::getChildren($aChild['id_category'], (int)$id_lang, true, (int)$id_shop), (int)$id_lang, true, (int)$id_shop)) {
        return true;
      }
    }
	...
	private function getCategory($id_category, $id_lang = false, $id_shop = false)
	{
		...
		$children = Category::getChildren((int)$id_category, (int)$id_lang, true, (int)$id_shop);
    $isInPath = $this->isSelected($id_category, $children, (int)$id_lang, true, (int)$id_shop);
    $isCurrent = $this->page_name == 'category' && ((int)Tools::getValue('id_category') == $id_category);

    $has_children = count($children);

    $class = 'category';
    $class .= $isCurrent ? ' sfHoverForce' : '';
    $class .= $isInPath ? ' selected' : '';
    $class .= $has_children ? ' parent ' : '';
		...
		{
      $sImg = '<span class="cat_icon" style="background-image:url(' . $this->context->link->getCatImageLink($category->link_rewrite, $category->id) . ');"></span>';
			$this->_menu .= '<li class="' . $class . '">';
			$this->_menu .= '<a href="'.$category_link.'">' . ($category->level_depth == 2 ? $category->name . $sImg : $sImg . '<span class="title">' . $category->name . '</span>') . '</a>';

			if (count($children))
			{
				$this->_menu .= '<ul class="child">';
	...
	protected function getCacheId($name = null)
	{
		...
		return 'blockuppermenu|'.(int)Tools::usingSecureMode().'|'.$page_name.'|'.(int)$this->context->shop->id.'|'.implode(', ',$this->user_groups).'|'.(int)$this->context->language->id.'|'.(int)Tools::getValue('id_category').'|'.(int)Tools::getValue('id_manufacturer').'|'.(int)Tools::getValue('id_supplier').'|'.(int)Tools::getValue('id_cms').'|'.(int)Tools::getValue('id_product');
	}

	public function hookUpperMenu($params)
	{
		$this->user_groups =  ($this->context->customer->isLogged() ? $this->context->customer->getGroups() : array(Configuration::get('PS_UNIDENTIFIED_GROUP')));
		$this->page_name = Dispatcher::getInstance()->getController();
		if (!$this->isCached('blockuppermenu.tpl', $this->getCacheId()))
		{
			$this->makeMenu();
            $result = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS(
                'SELECT id_seo, seo_url, title FROM '._DB_PREFIX_.'pm_advancedsearch_seo_lang WHERE title IN ("Распродажа", "Ротация", "Новые") AND id_lang = ' . (int)Configuration::get('PS_LANG_DEFAULT'));
            if ($result) {
                foreach ($result as $row) {
                    switch ($row['title']) {
                        case "Распродажа":
                            $onsaleLink = _PS_BASE_URL_ . '/s/' . $row['id_seo'] . '/'. $row['seo_url'];
                            break;
                        case "Ротация":
                            $hitLink = _PS_BASE_URL_ . '/s/' . $row['id_seo'] . '/'. $row['seo_url'];
                            break;
                        case "Новые":
                            $newLink = _PS_BASE_URL_ . '/s/' . $row['id_seo'] . '/'. $row['seo_url'];
                            break;
                    }
                }
            }
            $this->smarty->assign(array(
                    ...
                    'onsaleLink' => isset($onsaleLink) ? $onsaleLink : '',
                    'hitLink' => isset($hitLink) ? $hitLink : '',
                    'newLink' => isset($newLink) ? $newLink : ''
                )
            );
		}

		$html = $this->display(__FILE__, 'blockuppermenu.tpl', $this->getCacheId());
		return $html;
	}
...
//    $this->context->controller->addJS($this->_path.'js/superfish-modified.js');
    $this->context->controller->addCSS(($this->_path).'css/blockuppermenu.css');
  }
	...
	{
		$this->_clearCache('blockuppermenu.tpl');
	}
	
	public function hookActionShopDataDuplication($params)
	{
		$linksmenuupper = Db::getInstance()->executeS('
			SELECT *
			FROM '._DB_PREFIX_.'linksmenuupper 
			WHERE id_shop = '.(int)$params['old_id_shop']
			);

		foreach($linksmenuupper as $id => $link)
		{
			Db::getInstance()->execute('
				INSERT IGNORE INTO '._DB_PREFIX_.'linksmenuupper (id_linksmenuupper, id_shop, new_window) 
				VALUES (null, '.(int)$params['new_id_shop'].', '.(int)$link['new_window'].')');
			
			$linksmenuupper[$id]['new_id_linksmenuupper'] = Db::getInstance()->Insert_ID();
		}
		...
		foreach($linksmenuupper as $id => $link)
		{
			$lang = Db::getInstance()->executeS('
					...
					FROM '._DB_PREFIX_.'linksmenuupper_lang 
					WHERE id_linksmenuupper = '.(int)$link['id_linksmenuupper'].' AND id_shop = '.(int)$params['old_id_shop']);
			
			foreach($lang as $l)
				Db::getInstance()->execute('
					INSERT IGNORE INTO '._DB_PREFIX_.'linksmenuupper_lang (id_linksmenuupper, id_lang, id_shop, label, link) 
					VALUES ('.(int)$link['new_id_linksmenuupper'].', '.(int)$l['id_lang'].', '.(int)$params['new_id_shop'].', '.(int)$l['label'].', '.(int)$l['link'].' )');
		}
